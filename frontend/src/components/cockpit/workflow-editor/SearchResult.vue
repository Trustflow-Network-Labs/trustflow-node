<template>
  <main :class="cockpitWorkflowEditorSearchResultClass">
    ...
    <div :class="['service', {'data': service.type == 'DATA', 'function': service.type == 'DOCKER EXECUTION ENVIRONMENT' || service.type == 'STANDALONE EXECUTABLE'}]">
      ...
      <div class="service-content">
          <div class="service-content-section"
            ...
            <div class="service-content-section-header">{{ $t("message.cockpit.detail.workflow-editor.search-result.description") }}:</div>
            {{ shorten(service.description, 100, 0) }}
          </div>
          <div class="service-content-section">
            <div class="service-content-section-header">{{ $t("message.cockpit.detail.workflow-editor.search-result.service-price-model") }}:</div>
            <div class="service-price upper" v-if="service.service_price_model == null">{{ $t("message.cockpit.detail.workflow-editor.search-result.free") }}</div>
            <div v-else>
          ...
          <div class="service-content-section-group-header">{{ $t("message.cockpit.detail.workflow-editor.search-result.service-interfaces") }}:</div>
          <div class="service-content-section ind-1" v-for="(intfce, intfceIndex) in service.interfaces" :key="intfceIndex">
            <div class="service-content-section-data ind-m-1"
              v-if="intfce.interface_type == 'STDIN'">{{ "(" + (intfceIndex+1) + ")" }} {{ $t("message.cockpit.detail.workflow-editor.search-result.input-data") }}</div>
            <div class="service-content-section-data ind-m-1"
              v-else-if="intfce.interface_type == 'STDOUT'">{{ "(" + (intfceIndex+1) + ")" }} {{ $t("message.cockpit.detail.workflow-editor.search-result.output-data") }}</div>
            <div class="service-content-section-data ind-m-1"
              v-else-if="intfce.interface_type == 'MOUNT'">{{ "(" + (intfceIndex+1) + ")" }} {{ $t("message.cockpit.detail.workflow-editor.search-result.file-system-mount-point") }}</div>
            <div class="service-content-section-data ind-m-1"
              v-else>{{ "(" + (intfceIndex+1) + ") " + intfce.interface_type }}</div>
            <div class="service-content-section-header"
              v-if="intfce.description !=''">{{ $t("message.cockpit.detail.workflow-editor.search-result.interface-description") }}:</div>
            <div v-if="intfce.description !=''">{{ intfce.description }}</div>
            <div class="service-content-section-header">{{ $t("message.cockpit.detail.workflow-editor.search-result.interface-paths") }}:</div>
...
<script src="../../../js/cockpit/workflow-editor/search-result.js" scoped />
<style src="../../../scss/cockpit/workflow-editor/search-result.scss" lang="scss" scoped />
